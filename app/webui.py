...
<body>
  ...
  <a href="/api/update-stderr" target="_blank">교체 스크립트 원본 출력 보기</a>
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802
            ...
            if self.path == "/api/update-log":
                ...
                return
            if self.path == "/api/update-stderr":
                stderr_path = context.app_dir / "data" / "update-apply-stderr.log"
                body = "\n".join(_tail_log_lines(stderr_path)).encode("utf-8")
                self.send_response(200)
                self.send_header("Content-Type", "text/plain; charset=utf-8")
                self.send_header("Content-Length", str(len(body)))
                self.end_headers()
