...
빠져 있으면 간단한 웹 설정 페이지(setup_server.py)에서 몇 가지 값만 입력받아
이 모듈이 전체 config를 만들어낸다. 나머지 값(포트 등)은 rethink 원본
config.jsonc의 권장 기본값을 그대로 쓴다 — 443/8883에서 벗어나면 리다이렉트
이후 기기가 광고받은 새 포트로 옮겨가면서 우리 WinDivert 필터를 벗어날 수
있어서, 일부러 건드리지 않는다.
"""
